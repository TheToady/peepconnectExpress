...
function disconnectHandler(socket, thisUser){
    ...
    socket.on('disconnect', function () {
        if(thisUser.connectedTo != null){
            console.log('\nUSER: '.cyan + `${socket.id.cyan} ended the call with ${thisUser.connectedTo.cyan}`)
...
buttonHandler = (socket, thisUser, io) => {
    socket.on('button-next', () => {
        ...
        console.log('\nUSER: '.cyan + `${socket.id} searching for a new match`)
    ...
    socket.on('button-search', () => {
        console.log('\nUSER: '.cyan + `${socket.id} is now searching`)
        searchingUsers.push(userSessions[userFinder(userSessions, socket.id)])
        io.to(socket.id).emit('receive-search')
    })
    ...
    socket.on('button-stop', () => {
        if(thisUser.connectedTo == null){
            console.log('\nUSER: '.cyan + `${socket.id} is not searching anymore`);
        }else{
            console.log('\nUSER: '.cyan + `${socket.id} ended call with ${thisUser.connectedTo}`);
        }
        stopCall(socket, thisUser, io);
    })
}
...
function matchUser() {
    for(i = 0; i < searchingUsers.length; i++){
        if(i%2!=0){
            ...
            console.log(`\nMATCHED: ${searchingUsers[lastUserId].id} and ${searchingUsers[i].id} matched`)
